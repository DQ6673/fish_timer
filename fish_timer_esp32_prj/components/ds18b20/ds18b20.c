...
void ds18b20_write(char bit)
{
	if (bit & 1)
	{
		gpio_set_direction(DS18B20_IO_NUM, GPIO_MODE_OUTPUT);
		noInterrupts();
		gpio_set_level(DS18B20_IO_NUM, 0);
		ets_delay_us(6);
		gpio_set_direction(DS18B20_IO_NUM, GPIO_MODE_INPUT); // release bus
		ets_delay_us(64);
		interrupts();
	}
	else
...
unsigned char ds18b20_read(void)
{
	...
	gpio_set_level(DS18B20_IO_NUM, 0);
	ets_delay_us(6);
	gpio_set_direction(DS18B20_IO_NUM, GPIO_MODE_INPUT);
	ets_delay_us(9);
	value = gpio_get_level(DS18B20_IO_NUM);
	ets_delay_us(55);
	interrupts();
	return (value);
}
...
unsigned char ds18b20_reset(void)
{
	...
	gpio_set_level(DS18B20_IO_NUM, 0);
	ets_delay_us(480);
	gpio_set_level(DS18B20_IO_NUM, 1);
	gpio_set_direction(DS18B20_IO_NUM, GPIO_MODE_INPUT);
	ets_delay_us(70);
	presence = (gpio_get_level(DS18B20_IO_NUM) == 0);
	ets_delay_us(410);
	interrupts();
	return presence;
}
...
void ds18b20_requestTemperatures()
{
	...
	start = start / 1000ULL;
	while (!isConversionComplete() && ((end / 1000ULL) - start < millisToWaitForConversion()))
	{
...
float ds18b20_get_temp(void)
{
	if (init == 1)
	{
		...
		if (check == 1)
		{
			...
			check = ds18b20_RST_PULSE();
			float temp = 0;
			temp = (float)(temp1 + (temp2 * 256)) * 0.0625;
			return temp;
		}
		else
		{
...
{
	esp_rom_gpio_pad_select_gpio(DS18B20_IO_NUM);
	init = 1;
}
